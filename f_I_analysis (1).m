...
fp_data = 'C:\Users\schum\Google_Drive\Lab\Data\culture_experiments\fI\';

%location of the analyzed fI data
fp_analyzed_data = ...,
...
%duration of the current injection (in seconds)
pulse = 0.5;
...
for jj = 1:1%numel(experiment)
    ...
    %name of the saved analyzed data mat file
    save_file_name = strcat('fI_', current_experiment,sub,'.mat');
        ...
        if isempty(cell_id{1,ci}) == 1
            ...
            for ti = trace_start:cell_id{1,ci}(end,1)
                if ismember(ti,cell_id{1,ci}) == 0
                    MFR{1,ci}(ti,1) = NaN;
                    ISI{1,ci}(ti,1:cti) = NaN;
                    IFR{1,ci}(ti,1:cti) = NaN; 
                    V_th{1,ci}{ti} = {};
                    f_trough{1,ci}{ti} = {};
                    dV_sec{1,ci}{ti} = {};
                    AP_peak{1,ci}{ti} = NaN;
                    ...
                    udratio{1,ci}(ti,1:cti) = NaN;
                    width{1,ci}(ti,1:cti) = NaN;
    ...
    %                 end
                    ...
                    [dV_sec_temp, V_th_temp, f_trough_temp, sp_num_temp, udratio_temp] = get_vthresh(data, pulse, plot_on);
    ...
    %                         plot(fm2)
        ...
        Vrm_stat(ci,3) = abs(Vrm_stat(ci,1)-v_hold)/abs(v_hold)*100;
...
%     display_name = strcat(current_dir_name(total_dir_name_number-5: end),'.mat');
    ...
    'f_trough','f_trough_ave','dV_sec','AP_peak','AP_peak_ave','AP_peak_1st','Vrm','Vrm_stat','spike_count',...
